...
- PTY/tool reflow must preserve explicit status color on the `•` prefix; apply action/tool styling only to the verb so success, failure, and warning remain visually distinct; fullscreen `Ctrl+T` opens transcript review but remains text transpose outside fullscreen, with `Alt+O` as a compatibility alias, and complete PTY captures remain sidecars behind bounded live lines.
