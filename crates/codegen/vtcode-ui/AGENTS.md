...
- Tool and PTY blocks reserve at least one blank line above and below; shell syntax highlighting is accepted only when it produces distinct token colors, otherwise semantic token styles are the fallback. Diff rendering is foreground-only across modal preview, markdown, reflow, and ANSI output; overlays cache one `vtcode_diff::DiffDocument` at open time and only relayout or scroll it.
