...
- PTY/tool reflow must preserve explicit status color on the `•` prefix; apply action/tool styling only to the verb so success, failure, and warning remain visually distinct; fullscreen `Ctrl+T` opens the session-local tool-output viewer but remains text transpose outside fullscreen, and complete PTY captures stay behind bounded live lines.
