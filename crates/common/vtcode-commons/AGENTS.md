...
- `formatting/` owns the canonical middle-truncation helpers `truncate_middle` (head+tail, control chars sanitized) and `truncate_path_middle` (separator-aware, for path display), plus `lowercase_leading_word` for splicing a reason into a sentence (skips acronyms and "I") and shell-aware `wrap_shell_command` (quote-atomic, continuation-width-safe) alongside `wrap_text_words`. Downstream crates delegate here — do not re-implement per crate.
