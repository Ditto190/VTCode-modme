...
| `agent.harness.continuation.cross_turn_turns` | `integer` | no | `8` | Bounded cross-turn auto-continue turns after a recoverable end (budget/preview/tool-free recovery) or on resume while tracker work remains. `0` disables cross-turn tracker auto-queue (in-turn continuation still applies when `auto_continue_tracker` is true). |
