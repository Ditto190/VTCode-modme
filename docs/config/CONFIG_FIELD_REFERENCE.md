...
| `ui.thinking_display` | `string` | no | `"collapsed"` | Default collapse state of agent thinking/reasoning blocks. Options: "collapsed", "extended". When collapsed, each reasoning run shows a single `Thinking` summary line; click (or the toggle action) to expand. Expanded blocks show a `Thinking` header followed by the dimmed, indented reasoning body. |
