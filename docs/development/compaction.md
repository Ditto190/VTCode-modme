...
Every strategy bounds its input with `bound_history_for_summarization` against
`compaction_history_budget` (window minus overhead), keeping the newest complete
protocol groups and degrading to previews when no group fits. Before bounding,
...
cannot shrink the input. Results are bounded on output with
