...
- Anti-blind editing is session-scoped: once successful mutations require verification, a blocked turn and later `continue` retain the gate. Only a successful verification-classified command clears it; reads, diffs, failed checks, and ordinary stall recovery do not. Copilot/direct execution and batch execution must synchronize `SessionStats::verification_pending` immediately after updating their `LoopTracker`, so resumed turns reconstruct the same gate state.
...
- Compact tool summaries are session-local and per-call; compact mode is the default, command calls use their original `• Ran ...` header with bounded live PTY output, and failures, warnings, diffs, and result bodies retain their boundaries. Complete command output is available only in the fullscreen `Ctrl+T` tool-output viewer.
