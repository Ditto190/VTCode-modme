...
-   **GPT-6 Astra (`gpt-6-astra`):** most capable OpenAI model for end-to-end agentic work (1,050,000 context, 128k max output, reasoning `low`/`medium`/`high`/`xhigh`/`max`). Per the [latest-model guide](https://developers.openai.com/api/docs/guides/latest-model), VT Code routes it through the Responses API (tool calling requires Responses), coerces `none`/`minimal` reasoning effort to `low`, never sends `temperature`/`top_p`/`logprobs`, uses `prompt_cache_options.ttl: "30m"` instead of `prompt_cache_retention`, and appends a GPT-6 Astra prompt addendum (bias to action, user instructions outrank skills, parallel subagent delegation, proportionate testing).
