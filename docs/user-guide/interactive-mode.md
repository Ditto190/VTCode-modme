...
- When a turn is blocked (repeated tool denials hit the fuse), the TUI shows a `Blocked` header badge, `Blocked • continue to retry • /resume • details: .vtcode/tasks/current_blocked.md` footer hint, and a two-line transcript banner (short stop marker + one action). Type `continue` with new guidance, describe alternative instructions, or run `vtcode --resume <session>` from a terminal. Full reason, archive path, and last-turn forensics stay in `.vtcode/tasks/current_blocked.md`. One attempt before the fuse trips, the runtime warns with the remaining attempts and a per-tool remedy hint.
...
- `Enter` steers the active turn: the message is injected into the conversation right after the current tool-call batch, so the model sees it on its next request within the same turn. If the turn ends before another tool call runs, the steered message is delivered at the next turn boundary instead — nothing is lost. The transcript confirms genuine user steering with a `Steered into active turn: …` status line. Harness-generated continuations (tracker/plan auto-continue, background completion, verification recovery) stay quiet and never echo their full internal prompt.
