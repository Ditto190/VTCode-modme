...
| `Enter`                                     | Idle: submit the current draft. Active: steer the running turn — the message is injected into the conversation right after the current tool-call batch, so the model sees it on its next request within the same turn. | Plain input box only. Slash commands keep their normal routing while busy (immediate, queued, or blocked). |
...
| `Ctrl+Enter`                                | Idle: submit the current draft immediately (or the newest queued message if the draft is empty). Active: joins the visible queue as a *batchable* message; consecutive text-only messages are batched into one turn dispatched after the current turn. `/stop`, `/pause`, and `/resume` are handled immediately instead of being queued. | Plain input box only. |
...
- `/tasks` toggles the dedicated TODO panel. It is fed directly from `task_tracker` output and remains independent from the tool-summary display mode. Successful updates use the same compact tree in the transcript and panel; rows render inline markdown (code spans, emphasis, file paths) instead of raw source, inline `-> files:` / `-> verify:` suffixes are stripped into structured tracker metadata, and generated plan IDs (`1789108823046-kind-lagoon`) display humanized (`Kind Lagoon`) in the panel header only. The transcript uses a live summary header (`• Tasks 3/8 — next: …`) so random slugs never appear inline and identical repeats are collapsed to one block. Tool blocks render borderless without leading/trailing pipe framing. The panel header carries the checklist title and `completed/total` progress while step metadata remains available in structured tracker data. `Alt+G` toggles the panel from anywhere, and `ui.show_task_panel` controls whether it auto-shows when a plan is approved.
