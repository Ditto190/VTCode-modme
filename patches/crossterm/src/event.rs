...
pub enum Event {
    ...
    Resize(u16, u16),
    /// The terminal's color-scheme mode changed (Contour VT extension,
    /// unsolicited `CSI ? 997 ; Ps n` DSR enabled via `CSI ? 2031 h`).
    ///
    /// See <https://contour-terminal.org/vt-extensions/color-palette-update-notifications/>
    ColorSchemeReport {
        /// `true` when the terminal switched to dark mode, `false` for light.
        dark: bool,
    },
}
