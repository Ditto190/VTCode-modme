...
pub(super) fn render_secret_usage(renderer: &mut AnsiRenderer) -> Result<()> {
    ...
    renderer.line(
        MessageStyle::Info,
        "Keys are stored in your OS keyring (with encrypted-file fallback) — never in workspace environment files.",
    )?;
    renderer.line(
        MessageStyle::Info,
    ...
    renderer.line(
        MessageStyle::Info,
        "Examples: /secret, /secret status openrouter, /secret add anthropic, /secret delete openai, /secret migrate",
    )?;
    Ok(())
}
