...
const POST_TOOL_RECOVERY_REASON_PLAN_MODE: &str = "Planning research completed, but final plan synthesis needs recovery. Tools are disabled. Produce the `<proposed_plan>` NOW from the context and tool outputs already in this conversation: include Summary, numbered Implementation Steps (one line each: `Action -> files: [src/parser.rs] -> verify: [cargo check --locked]`), Test Cases and Validation, and Assumptions and Defaults. Prefer file:symbol references, and do NOT emit any tool calls or tool-call markup. Keep any text outside the block to one line or omit it; it is ignored.";
...
const RECOVERY_TOOL_CALL_RETRY_DIRECTIVE_PLAN_MODE: &str = "Recovery: in plan mode, tools are disabled and you must finalize the plan. Emit the `<proposed_plan>` now from the research already gathered in this conversation — Summary, numbered steps on single lines (`Action -> files: [src/parser.rs] -> verify: [cargo check --locked]`), Validation, Assumptions — with at most one intro line outside the block, no tool calls, and no `<tool_call>`/`<invoke>`/`<function=...>` markup.";
