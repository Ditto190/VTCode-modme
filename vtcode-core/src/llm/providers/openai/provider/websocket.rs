...
// Legacy mirror of the vtcode-llm optional WebSocket transport.
// Active OpenAI provider construction now routes through vtcode-llm; this copy
// remains until the duplicate core provider is deleted. Keep the semantics
// identical: WebSocket mode is separate from normal HTTP/SSE, may use
// `previous_response_id` only as connection-local state, and always sends
// `store=false`.
